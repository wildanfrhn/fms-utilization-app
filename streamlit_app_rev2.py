...
st.caption("Upload Data FMS, pilih bulan output, pilih hari libur, lalu download hasil Data Utilisasi.")
...
def month_options_from_data(df_utilisasi):
    ...
    temp = temp.drop_duplicates().sort_values(["Year", "Month_Num"])
...
def build_summary(df_output, target_days):
    ...
    summary["Check"] = np.where(summary["Total Status"] <= int(target_days), "OK", "Melebihi Target Days")
...
with st.sidebar:
    ...
    st.info("Urutan prioritas status: Utilized > Standby > Permit > PM > RM")
...
try:
    ...
    warning_count = int((df_summary["Check"] == "Melebihi Target Days").sum()) if not df_summary.empty else 0
    if warning_count > 0:
        st.warning(f"Ada {warning_count} Lic Number yang total statusnya masih melebihi Target Days. Cek sheet Summary.")
    else:
        st.success("Semua Lic Number tidak melebihi Target Days.")
